...
def normalize_path(f):
    ...
    def f_normalize_path(*args, **kwargs):
        if 'path' in kwargs:
            if kwargs['path']:
                kwargs['path'] = '/applications/' + kwargs['path']
            else:
                kwargs['path'] = '/applications'
...
class Controller(object):
    ...
    def index(self, request, template_id, path):
        LOG.debug('Tiers:Get <TemplateId: {0}, '
    ...
    def post(self, request, template_id, path, body):
        ...
        LOG.debug('Tiers:Post <TempId: {0}, Path: {2}, '
                  'Body: {1}>'.format(template_id, secure_data, path))

        post_data = core_services.CoreServices.post_template_data
        try:
    ...
    def put(self, request, template_id, path, body):
        LOG.debug('Tiers:Put <TempId: {0}, Path: {2}, '
